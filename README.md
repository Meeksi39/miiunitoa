...
- **メニューにレイアウトがない** — まだ保存していません。`ml save <name>` を
  実行してください。
- **全画面アプリが違うモニターに飛ぶ／レイアウトの挙動がおかしい** — `ml save`
  実行時に *「monitors are indistinguishable by EDID」* という警告が出る場合、
  2 台以上のモニターが同じ vendor/product/serial を報告しています（安価なモニ
  ターでシリアルが `0x00000000` のままになっているとよく起きます）。GNOME は
  接続先（connector）名でしか区別できず、その名前は再起動で入れ替わることが
  あるため、保存したレイアウトが別の物理モニターに適用されてしまいます。確実な
  対処は、衝突しているモニターの 1 台にコネクター単位のカーネル EDID 上書き
  （`drm_kms_helper.edid_firmware=<connector>:edid/<file>.bin`）で固有のシリアル
  を持たせることです。
